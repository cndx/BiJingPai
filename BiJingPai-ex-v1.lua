...
_G.Config={
    standard = "WRC20",	
    owner = "wXYtbLww1eVBA9ytUnkPXQmNjoGV3HuFRs",  --WPmNonXuc2qzKxp5AEcYgsDawcaBEbjBb7 
    name = "BiJingPai.com",
    symbol = "BJP",
    decimals = 8,
...
----------------------------------------------------------------------------------------------
--https://BiJingPai.com  XYT/BTC/DOGE/BTA/WICC as BJP auction for BTCALL & YMBing.com v11.26
--Base on 3950364-1 add http://GiaoEX.com/  100wBJP-WICC
...
Send = function()
	...
	if tx.addr==EXaddress and curaddr~=_G.Config.owner then
		...
		local beishu=1000000  	--QKL _G.AppData.ReadInt("beishu")
		...
		if rates~=8888 and rates>=minrates and rates<=maxrates then
				AddEXlist(rates,curaddr,math.floor(nums/beishu))
			else
			if rates==8888 then
...
SetRates= function()
	...
	if curaddr~=_G.Config.owner then --QKL   ==  test ~=
		local tx=_G.Hex:New(contract):Fill({"w",4,"exts",4,"minp",4,"maxp",4,"maxlen",4,"minnum",4,"maxnum",8,"beishu",4,"giao",""..(#contract-36)})
		--"exts",7,"minp",1,"maxp",9999,"maxlen",300,"minnum",10000,"maxnum",1000000000000,"beishu",1000000,"giao","mBJP-WICC"
		...
		_G.AppData.Write("beishu",tx.beishu)
...
Wicc2Btcall= function()	
	...
	if NetWicc > 100000 then
		...
		local beishu=1000000  	--QKL _G.AppData.ReadInt("beishu")
		_G.Asset.SendAppAsset(EXaddress,curaddr,math.floor(getbta*beishu))
...
function SaveEXlist(ex)
	...
	for i=4,#ex-3,3 do
		...
		if ex[i]==ex[i+3] and ex[i+1]==ex[i+4] then
			exlist=exlist.."|"..ex[i].."|"..ex[i+1].."|"..math.floor(0+ex[i+2]+ex[i+5])
			i=i+3
		else
...
,"f081011807000000010000000f2700002c010000102700000010a5d4e800000040420f006d424a502d57494343"
